...
/*-------------------------------------------------------------*/
void multiplica_matriz(int ini, int pulo, float matA[N][N], float matB[N][N], float *matC){
	int i,j,k;
...
int main(int argc, char **argv){
	...
	if(pid > 0 ){
		multiplica_matriz(0,2,matA,matB,matC);
		wait(NULL);
		...
		multiplica_matriz(1,2,matA,matB,matC);
		shmdt(matC);
	}
